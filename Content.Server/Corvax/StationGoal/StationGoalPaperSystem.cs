    ...
    public sealed class StationGoalPaperSystem : EntitySystem
    {
        ...
        private void OnRoundStarting(RoundStartingEvent ev)
        {
            ...
            while (query.MoveNext(out var uid, out var station))
            {
                ...
                if (!SendStationGoal(uid, selGoal))
                {
                    Log.Info($"Goal {selGoal.ID} not has been sent to station {MetaData(uid).EntityName}");
                }
        ...
        public bool SendStationGoal(EntityUid? ent, StationGoalPrototype goal)
        {
            ...
            var printout = new FaxPrintout(
                ...
                {
                    new StampDisplayInfo { StampedName = Loc.GetString("stamp-component-stamped-name-centcom"), StampedColor = Color.FromHex("#3d642d") },
                }
