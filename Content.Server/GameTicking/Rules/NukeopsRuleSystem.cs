...
#region Event Handlers
    ...
    private void OnPlayersSpawning(RulePlayerSpawningEvent ev)
    {
        ...
        while (query.MoveNext(out var uid, out _, out var nukeops, out _))
        {
            ...
            var commanderEligible =
                _antagSelection.GetEligibleSessions(ev.PlayerPool, nukeops.CommanderSpawnDetails.AntagRoleProto);

            var agentEligible =
                _antagSelection.GetEligibleSessions(ev.PlayerPool, nukeops.AgentSpawnDetails.AntagRoleProto);

            var operativeEligible =
                _antagSelection.GetEligibleSessions(ev.PlayerPool, nukeops.OperativeSpawnDetails.AntagRoleProto);

            //Calculate how large the nukeops team needs to be
            var nukiesToSelect = _antagSelection.CalculateAntagCount(_playerManager.PlayerCount,
                nukeops.PlayersPerOperative, nukeops.MaxOps);

            //Select Nukies
            //Select Commander, priority : commanderEligible, agentEligible, operativeEligible, all players
            var selectedCommander = _antagSelection
                .ChooseAntags(1, commanderEligible, agentEligible, operativeEligible, ev.PlayerPool).FirstOrDefault();

            //Select Agent, priority : agentEligible, operativeEligible, all players
            var selectedAgent = _antagSelection.ChooseAntags(1, agentEligible, operativeEligible, ev.PlayerPool)
                .FirstOrDefault();
            ...
            for (var i = 0; i < nukiesToSelect - 2; i++)
            {
                ...
                operatives.Add(selectedOperatives.Count > i
                    ? new NukieSpawn(selectedOperatives[i], nukeops.OperativeSpawnDetails)
                    : new NukieSpawn(null, nukeops.OperativeSpawnDetails));
            }
    ...
    private void OnShuttleCallAttempt(ref CommunicationConsoleCallShuttleAttemptEvent ev)
    {
        ...
        while (query.MoveNext(out _, out _, out var nukeops, out _))
        {
            ...
            if (nukeops is not { WarDeclaredTime: not null })
                continue;
    ...
    private void DistributeExtraTc(NukeopsRuleComponent nukieRule)
    {
        ...
        while (enumerator.MoveNext(out var uid, out var component))
        {
            ...
            if (Transform(uid).MapID !=
                Transform(nukieRule.NukieOutpost.Value).MapID) // Will receive bonus TC only on their start outpost
                continue;

            _store.TryAddCurrency(
                new Dictionary<string, FixedPoint2> { { TelecrystalCurrencyPrototype, nukieRule.WarTCAmountPerNukie } },
                uid,
                component);
    ...
    private void OnRoundStart(EntityUid uid, NukeopsRuleComponent? component = null)
    {
        ...
        component.OperationName =
            _randomMetadata.GetRandomFromSegments([OperationPrefixDataset, OperationSuffixDataset], " ");
    ...
    /// </summary>
    private void SetupOperativeEntity(
        EntityUid mob,
        string name,
        NukeopSpawnPreset spawnDetails,
    ...
    private void SpawnOperatives(List<NukieSpawn> sessions, bool spawnGhostRoles, NukeopsRuleComponent component)
    {
        ...
        {
            var name =
                $"{Loc.GetString(nukieSession.Type.NamePrefix)} {RobustRandom.PickAndTake(_prototypeManager.Index(nukieSession.Type.NameList).Values.ToList())}";
            ...
            {
                var profile =
                    _prefs.GetPreferences(nukieSession.Session.UserId).SelectedCharacter as HumanoidCharacterProfile;
    ...
    private void NotifyNukie(ICommonSession session, NukeOperativeComponent nukeop, NukeopsRuleComponent nukeopsRule)
    {
        ...
        _antagSelection.SendBriefing(session,
            Loc.GetString("nukeops-welcome", ("station", station), ("name", nukeopsRule.OperationName)), Color.Red,
            nukeop.GreetSoundNotification);
    }
